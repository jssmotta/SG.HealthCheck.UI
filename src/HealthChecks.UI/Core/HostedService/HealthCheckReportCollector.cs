    ...
    private async Task<UIHealthReport> GetHealthReportAsync(HealthCheckConfiguration configuration)
    {
        ...
        try
        {
            ...
            if (!string.IsNullOrEmpty(absoluteUri.UserInfo))
            {
                ...
                {
                    //_httpClient.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Basic", Convert.ToBase64String(Encoding.ASCII.GetBytes($"{userInfoArr[0]}:{userInfoArr[1]}")));
                    ...
                    var credentials = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{userInfoArr[0]}:{userInfoArr[1]}"));
